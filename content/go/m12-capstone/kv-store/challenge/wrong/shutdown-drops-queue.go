...
func NewPool(workers, queue int) *Pool {
	p := &Pool{jobs: make(chan func(), queue), done: make(chan struct{}), quit: make(chan struct{})}
	var wg sync.WaitGroup
	for range workers {
		wg.Go(func() {
			// Checks quit before every job, so after Shutdown the jobs still
			// sitting in the queue are silently dropped.
			for {
				select {
				case <-p.quit:
					return
