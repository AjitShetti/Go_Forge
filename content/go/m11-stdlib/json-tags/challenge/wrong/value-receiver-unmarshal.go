...
// A value receiver: the decoder still finds and calls this method (it's in
// *Status's method set), but it sets a copy, so every order stays Pending.
...
func DecodeOrder(data []byte) (Order, error) {
	// json.Unmarshal ignores unknown fields. A Decoder can refuse them.
